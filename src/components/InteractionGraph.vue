...
const stylesheet = [
  {
    ...
    style: {
      ...
      'text-margin-y': 3,
      'text-opacity': 1,
      // Labels render only once they'd be legible on-screen; this hides them when zoomed out
      // (clean) and reveals readable names as you zoom into a region. Selected/highlighted
      // nodes (below) override this to always show their label.
      'min-zoomed-font-size': big ? 9 : 0,
  ...
  { selector: 'node.selected', style: { 'border-width': 4, 'border-color': '#ffffff', 'text-opacity': 1, 'min-zoomed-font-size': 0, 'font-size': 13, 'z-index': 99 } },
  { selector: '.dim', style: { opacity: 0.07, 'text-opacity': 0.04 } },
  { selector: 'edge.highlight', style: { opacity: 0.95, width: 2.6 } },
  { selector: 'node.highlight', style: { 'border-color': '#ffffff', 'border-width': 3, 'text-opacity': 1, 'min-zoomed-font-size': 0, 'z-index': 90 } },
